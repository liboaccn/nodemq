...
function removeValue(req,res){
  //lrange
  var query = url.parse(req.url,true).query;

  var value = query.value;   //typeof(s)!="undefined"&&s!=null
  if (!(typeof(value)!="undefined"&&value!=null)) {
    res.end("error:value is empty");
  };
  if (query.queueName) {

        db.lrem(query.queueName,'0',value,function(err,fffff){
          console.log(err);
          console.log(fffff);
             res.end('');
        });
        ...
        db.lrem("queue",'0',value,function(err,fffff){
            console.log(err);
            console.log(fffff);
            res.end('');
